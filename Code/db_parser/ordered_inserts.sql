...
CREATE TABLE public.planet_osm_polygon (
    ...
    z_order integer,
    way_area real,
    way public.geometry(Polygon,4326)
);
...
CREATE TABLE public.poi (
    ...
    context public.geometry(Geometry,4326)
);
...
CREATE MATERIALIZED VIEW public.pois_lines_information AS
 SELECT poi.id,
    ...
    public.st_length(public.st_transform(public.st_intersection(poi.context, lines_information.way), 4326)) AS length,
...
CREATE MATERIALIZED VIEW public.pois_polygons_building_information AS
 SELECT poi.id,
    ...
    sum((public.st_area(public.st_transform(public.st_intersection(poi.context, polygons_building_information.way), 4326)) * ((0.3048 ^ (2)::numeric))::double precision)) AS area_total
   FROM public.poi,
...
CREATE MATERIALIZED VIEW public.pois_roads_information AS
 SELECT poi.id,
    ...
    public.st_length(public.st_transform(public.st_intersection(poi.context, roads_information.way), 4326)) AS length,
