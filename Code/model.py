...
#     def __init__(self, num_cats, embedding_dim, pretrained_embeddings):
...
#         # If pre-trained embeddings are provided, use them to initialize the embedding layer
#         if pretrained_embeddings is not None:
#             print("Chegou os embeddings")
#             self.init_embeddings(pretrained_embeddings)

#     def init_embeddings(self, pretrained_embeddings):
#         # Ensure that pretrained_embeddings is a tensor
#         if isinstance(pretrained_embeddings, np.ndarray):
#             pretrained_embeddings = torch.from_numpy(pretrained_embeddings)
        
#         print(self.cat_embedding.weight.size())
#         print(pretrained_embeddings.size())
        
#         # Check if the dimensions match
#         assert self.cat_embedding.weight.size() == pretrained_embeddings.size(), \
#             "The dimensions of the pre-trained embeddings do not match with the embedding layer"
        
#         # Replace the initial weights of the embedding layer with the pre-trained embeddings
#         self.cat_embedding.weight = nn.Parameter(pretrained_embeddings)
...
"""class PositionalEncoding(nn.Module):
    def __init__(self, d_model=568, dropout=0.1, max_len=500):
        super(PositionalEncoding, self).__init__()
        self.dropout = nn.Dropout(p=dropout)

        # Matriz de encoding posicional
        ...
        # Projeta de 603 para 568 (ou qualquer que seja o d_model correto)
        self.linear_projection = nn.Linear(603, d_model)

    def forward(self, x):
        x = self.linear_projection(x)  # Ajusta para (seq_len, batch_size, d_model)

        # Garante que `self.pe` tenha o mesmo tamanho de `x`
...
class TransformerModel(nn.Module):
    def __init__(self, num_poi, num_cat, embed_size, nhead, nhid, nlayers, dropout=0.5):
        ...
        print(f"Embed size na inicialização do PositionalEncoding: {embed_size}")
