...
embeddings = "austin-sl-tuple-geoc2vec0bin-wgt0.8pfp-concat-c.model"
word2vec = Word2Vec.load(f'./geo2vec_embeddings/{embeddings}')
#lines = "new-york-sl-tuple-geoc2vec-pois_lines_informationpfp-c.model"
#roads = "new-york-sl-tuple-geoc2vec-pois_roads_informationpfp-c.model"
#points = "new-york-sl-tuple-geoc2vec-pois_points_informationpfp-c.model"
#polygons = "new-york-sl-tuple-geoc2vec-pois_polygons_informationpfp-c.model"
lines = "new-york-sl-tuple-geoc2vec-μ90pois_lines_informationpfp-c.model"
roads = "new-york-sl-tuple-geoc2vec-μ90pois_roads_informationpfp-c.model"
points = "new-york-sl-tuple-geoc2vec-μ90pois_points_informationpfp-c.model"
polygons = "new-york-sl-tuple-geoc2vec-μ90pois_polygons_informationpfp-c.model"
lines_bert = "bert_reduced_lines_256.model"
roads_bert = "bert_reduced_roads_256.model"
points_bert = "bert_reduced_points_256.model"
polygons_bert = "bert_reduced_polygons_256.model"



#lines_bert = "new-york-sl-tuple-geoc2vec-μ90-pois-distilw2v-lines_information-pfp-c.model"
#roads_bert = "new-york-sl-tuple-geoc2vec-μ90-pois-distilw2v-roads_information-pfp-c.model"
#points_bert = "new-york-sl-tuple-geoc2vec-μ90-pois-distilw2v-points_information-pfp-c.model"
#polygons_bert = "new-york-sl-tuple-geoc2vec-μ90-pois-distilw2v-polygons_information-pfp-c.model"
word2vec = Word2Vec.load(f'./geo2vec_embeddings/{embeddings}')
#model_lines = Word2Vec.load(f'./db_parser/geographic/new_mi/model/{lines}')
#model_points = Word2Vec.load(f'./db_parser/geographic/new_mi/model/{points}')
#model_roads = Word2Vec.load(f'./db_parser/geographic/new_mi/model/{roads}')
#model_polygons = Word2Vec.load(f'./db_parser/geographic/new_mi/model/{polygons}')
model_lines = Word2Vec.load(f'./db_parser/geographic/new_mi/model/Bert/PCA/{lines_bert}')
model_points = Word2Vec.load(f'./db_parser/geographic/new_mi/model/Bert/PCA/{points_bert}')
model_roads = Word2Vec.load(f'./db_parser/geographic/new_mi/model/Bert/PCA/{roads_bert}')
model_polygons = Word2Vec.load(f'./db_parser/geographic/new_mi/model/Bert/PCA/{polygons_bert}')
model_shortest = Word2Vec.load(f'./ShortestPath/NYC-sl-tuple-geoc2vec-shortest-path-pfp-c.model')

# cat_id_dict = pd.read_csv("./dataset/NYC/POI_catid_to_yelp_category.csv").set_index("POI_catid").to_dict()["yelp_category"]
#cat_id_dict = pd.read_csv("./dataset/CA/cat_ids_to_category.csv").set_index("POI_catid_code").to_dict()["POI_catname"]
cat_id_dict = pd.read_csv("./dataset/NYC/cat_ids_to_category.csv").set_index("POI_catid").to_dict()["POI_catname"]

# poi_id_2_cat_dict = pd.read_csv("./dataset/NYC/poi_ids_2_yelp_cat.csv").set_index("POI_id").to_dict()["yelp_category"]

def get_custom_embeddings(cat_id):
    category = cat_id_dict[cat_id]
    embed_lines = get_embedding_or_default(model_lines, category)
    embed_points = get_embedding_or_default(model_points, category)
    embed_roads = get_embedding_or_default(model_roads, category)
    embed_polygons = get_embedding_or_default(model_polygons, category)
    #embed_shortest = get_embedding_or_default(model_shortest, category)
    #embed_mid = np.concatenate([embed_lines, embed_points, embed_roads, embed_polygons], axis=0)
    #embed = np.concatenate([embed_mid, embed_shortest], axis=0)
    embed = np.concatenate([embed_lines, embed_points, embed_roads, embed_polygons], axis=0)    
    
    return torch.tensor(embed).to(device=args.device)
    
def get_embedding_or_default(model, category):
    try:
        # Tenta buscar o embedding da categoria
        return model.wv[category]
    except KeyError:
        # Se não encontrar, retorna um vetor de zeros com a dimensão do embedding
        vector_size = model.wv.vector_size
        return np.zeros(vector_size)

#def get_custom_embeddings(cat_id):
#    category = cat_id_dict[cat_id].split(".")
#    embeddings = []
#    if (len(category) > 1):
#        for cat in category:
...
def train(args):
    ...
    # args.embeddings = f'{lines} \n {roads} \n {points} \n {polygons}'
    args.embeddings = f'{embeddings}'
    ...
    inv_cat_id2idx_dict = {v: k for k, v in cat_id2idx_dict.items()}  
    ...
    #     # pretrained_embeddings = np.array([get_custom_embeddings(poi_id) for poi_id in poi_ids])
    ...
    # size of the custom embeddings: GC2V = 140, SP = 35, GCSP = 175, BERT = 128
    args.seq_input_embed = args.poi_embed_dim + args.user_embed_dim + args.time_embed_dim + args.cat_embed_dim + 1024
    ...
    def input_traj_to_embeddings(sample, poi_embeddings):
            ...
            #                                       input_seq
                    # traj_id    poi_id    time              poi_id      time
        # Sample = ['626_40', [[3692, 0.3958333333333333], [1073, 0.3958333333333333]], [[1073, 0.3958333333333333], [4378, 0.4166666666666667]]]
        ...
        for idx in range(len(input_seq)):
            ...
            poi_catid = inv_cat_id2idx_dict[input_seq_cat[idx]]
            custom_poi_embeddings = get_custom_embeddings(poi_catid)
            ...
            #concat_embedding = torch.cat((fused_embedding1, fused_embedding2, custom_poi_embeddings), dim=-1)
            gcn_embedding = torch.cat((fused_embedding1, fused_embedding2), dim=-1)
            concat_embedding = torch.cat((gcn_embedding, custom_poi_embeddings), dim=-1)
            
            # print(f"Shape fused1 embeddings: {fused_embedding1.shape}")
            # print(f"Shape fused2 embeddings: {fused_embedding2.shape}")
            # print(f"Shape custom embeddings: {custom_poi_embeddings.shape}")
            # print(f"Shape concat embeddings: {concat_embedding.shape}")
