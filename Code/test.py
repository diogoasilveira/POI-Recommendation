...
lines_bert = "word2vec_reduced_lines_32.model"
roads_bert = "word2vec_reduced_roads_32.model"
points_bert = "word2vec_reduced_points_32.model"
polygons_bert = "word2vec_reduced_polygons_32.model"
...
model_shortest = Word2Vec.load(f'./ShortestPath/CA-sl-tuple-geoc2vec-shortest-path-pfp-c.model')
...
def test(args):
    ...
    pretrained_embeddings = np.array([get_custom_embeddings(poi_cat).cpu().numpy() for poi_cat in cat_ids])
    ...
    args.seq_input_embed = args.poi_embed_dim + args.user_embed_dim + args.time_embed_dim + args.cat_embed_dim #+ 175
    ...
    def input_traj_to_embeddings(sample, poi_embeddings):
        ...
        for idx in range(len(input_seq)):
            ...
            #device = gcn_embedding.device
            #custom_poi_embeddings = custom_poi_embeddings.to(device)  # Ensure it's on GPU
